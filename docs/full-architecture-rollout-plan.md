...
| Secrets | Key Vault private endpoint | `pe-kv-kairoai-hub-ci` `10.10.2.4` | Live | Approved private connection with centralized `privatelink.vaultcore.azure.net` DNS. |
...
| Secrets | Key Vault private endpoint | `pe-kv-kairoai-test-ci` `10.20.13.4` | Live | Approved private connection in `snet-private-endpoints`. |
...
| Secrets | Key Vault private endpoint | `pe-kv-kairoai-prod-ci` `10.30.13.4` | Live | Approved private connection in `snet-private-endpoints`. |
...
| Secrets | Key Vault private endpoint | `pe-kv-kairoai-prod-dr-si` `10.40.13.4` | Live | Approved private connection in the DR private-endpoint subnet. |
...
- Remaining private endpoints and stricter public network lock-down after AKS/runtime and private CI execution are fully validated.
...
4. Add remaining ACR, Service Bus, Storage, AI, and Monitor private endpoints; disable Key Vault public access after private runtime and CI connectivity is validated.
